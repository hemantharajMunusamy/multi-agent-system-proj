...
public:
    ...
    RepastHPCDemoModel(std::string propsFile, int argc, char** argv, boost::mpi::communicator* comm) {
        ...
        if(repast::RepastProcess::instance()->rank() == 0) {
            props->writeToSVFile("record.csv");
        }
    }
    ...
    void initSchedule(repast::ScheduleRunner& runner) {
        ...
        runner.scheduleEndEvent(repast::Schedule::FunctorPtr(
            new repast::MethodFunctor<RepastHPCDemoModel>(this, &RepastHPCDemoModel::recordResults)
        ));
    ...
    void recordResults() {
        if(repast::RepastProcess::instance()->rank() == 0) {
            ...
            props->writeToSVFile("result.csv", keyOrder);
        }
    }
};
...
int main(int argc, char** argv) {
    ...
    std::string propsFile = argv[2];  // The name of the properties file in Arg 2
